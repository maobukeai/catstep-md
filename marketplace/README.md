# Catstep MD on the MCP marketplaces

Submission materials + listing assets for getting `catstep-mcp` and the Catstep MD
...
Every listing leads back to **[github.com/maobukeai/catstep-md](https://github.com/maobukeai/catstep-md)** so MCP /
...
|---|---|---|---|
| **Smithery** ([smithery.ai](https://smithery.ai)) | [`smithery/`](smithery/) | `smithery.yaml` + Dockerfile + listing copy | ⏸️ **Deferred** — Smithery's web publish flow now requires an HTTP-accessible MCP endpoint (`https://your-server.com/mcp`); `catstep-mcp` is stdio JSON-RPC only. Revisit after we ship an HTTP/SSE transport wrapper. The `smithery.yaml` at repo root stays in place for any future passive discovery. |
...
| **Claude Code Skills** ([anthropic.com/claude-code](https://anthropic.com/claude-code)) | [`claude-code-skill/`](claude-code-skill/) | `solomd/SKILL.md` — installs `catstep-mcp` + documents the 11 recipes | ⏳ Ready to submit |
...
list and the Catstep MD pitch don't drift:

- [`_shared/tools.md`](_shared/tools.md) — the 13 MCP tools (8 generic + 5
  Catstep MD-only). Copy into any listing.
- [`_shared/about-solomd.md`](_shared/about-solomd.md) — the "What is
  Catstep MD?" block + download CTA. Goes at the top of every listing.
...
## Updating after a Catstep MD release

When you cut a new Catstep MD version (e.g. v4.4.2):
...
`catstep-mcp` knows nothing about recipes — recipes are interpreted by the
Catstep MD app's recipe runner, OR by hand-adapting the YAML to whatever your
