...
set -uo pipefail
DMG=$(ls /Volumes/Dev/code/notebook/app/src-tauri/target/universal-apple-darwin/release/bundle/dmg/CatstepMD_*_universal.dmg 2>/dev/null | head -1)
APP="/Volumes/Dev/code/notebook/app/src-tauri/target/universal-apple-darwin/release/bundle/macos/CatstepMD.app"
...
echo "=== Catstep MD signed-build verification ==="
echo "  dmg: $DMG"
echo "  app: $APP"
echo

chk ".app signed by Developer ID"     bash -c "codesign -dvvv '$APP' 2>&1 | grep -q 'Developer ID Application:'"
...
chk "binary is universal (x86_64+arm64)" bash -c "file '$APP/Contents/MacOS/Catstep MD' 2>&1 | grep -q 'universal binary'"
chk "x86_64 slice present"             bash -c "lipo -info '$APP/Contents/MacOS/Catstep MD' 2>&1 | grep -q x86_64"
chk "arm64 slice present"              bash -c "lipo -info '$APP/Contents/MacOS/Catstep MD' 2>&1 | grep -q arm64"
...
chk "catstep-mcp sidecar bundled"       test -f "$APP/Contents/MacOS/catstep-mcp"
chk "catstep-mcp sidecar signed"        bash -c "codesign -dvvv '$APP/Contents/MacOS/catstep-mcp' 2>&1 | grep -q 'Developer ID Application:'"
chk "dmg signed by Developer ID"       bash -c "codesign -dvvv '$DMG' 2>&1 | grep -q 'Developer ID Application:'"
MNT=$(hdiutil attach -nobrowse "$DMG" 2>&1 | tail -1 | awk '{print $NF}')
chk "dmg mounts + contains CatstepMD.app" test -d "$MNT/CatstepMD.app"
