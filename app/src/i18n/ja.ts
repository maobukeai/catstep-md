...
export const ja: I18n = {
  ...
  integrations: {
    ...
    cliCopiedToast:
      'インストールコマンドをコピーしました —— ターミナルに貼り付けて実行してください。',
    cliInstallToast:
      'ターミナルで次のコマンドを実行してください（/usr/local/bin への書き込みには sudo が必要です）：',
...
,
  ...
  rag: {
    ...
    enableHint:
      'デフォルトはオフ。オンにするとノートが <workspace>/.solomd/embeddings.sqlite にローカルで埋め込まれます — ネットワーク不要、同梱の hashed-trigram バックエンドはモデルダウンロードも不要。{key} でパネルを開きます。',
