export const en = {
  ...
  menubar: {
    ...
    themes: 'Themes',
    tools: 'Tools',
    // Menu item labels (used by Toolbar.vue menubarMenus)
  ...
  explorer: {
    ...
    folderTruncatedHint:
      'This folder has more than 10,000 entries; showing the first batch. Move groups into subfolders to see them all.',
  ...
  settings: {
    ...
    groupAboutApp: 'Software Information',
    ...
    quickCapture: 'Global Quick Capture',
    quickCaptureHint: 'Press hotkey in any application to open a lightweight scratchpad; press Enter to send thoughts directly into Inbox.',
    quickCaptureFailed: 'Failed to register shortcut: {error}',
    ...
    imageExportBranding: 'Append "Created with Catstep MD" footer when exporting as image',
    ...
    spellcheckLangHint: 'Only en_US ships with Catstep MD. Drop a Hunspell <code>.aff</code> + <code>.dic</code> pair (e.g. from the LibreOffice dictionaries) into that folder and it appears here.',
    ...
    setDefault: '⭐ Set Catstep MD as default Markdown editor',
    settingDefault: 'Setting…',
    setDefaultHint: 'Registers Catstep MD as the default app for .md / .markdown / .mdown / .mkd files. On Windows, also adds Catstep MD to the "Open with" menu. On Linux requires xdg-utils.',
    ...
    openLinkedFilesExternallyHint: 'When on, clicking a Markdown link to a local PDF / Office / other document opens it in the OS default app. Turn off to convert it to Markdown instead. Links to .md / text / images always open inside Catstep MD.',
  ...
  ai: {
    settingsHeading: 'AI Provider & Models (Global)',
    ...
    enable: 'Enable AI Core Features',
    enableHint: 'Off by default. When enabled, powers text rewrite, sidebar Agent, and recipes.',
  ...
  rag: {
    ...
    enableHint:
      'Off by default. When on, your notes are embedded locally into <workspace>/.solomd/embeddings.sqlite — no network, no model download needed for the bundled hashed-trigram backend. {key} opens the panel.',
    ...
    openFolder: 'Open a folder first ({folderKey} → Folder)',
    ...
    reindex: 'Reindex',
    hintNav: '↑↓ navigate · ↵ open · Tab switch to exact · Esc close',
    disabledHeading:
  ...
  inbox: {
    ...
    enableCaptureHint:
      'Off by default. When on, Catstep MD listens on 127.0.0.1 (loopback only) for new-note POSTs from a browser extension, iOS Shortcut, shell script, or curl. Notes land in the workspace inbox folder with `inbox: true` in their YAML.',
    endpoint: 'Endpoint',
  ...
  rest: {
    ...
    enableHint:
      'Off by default. When on, Catstep MD listens on 127.0.0.1 (loopback only). Every route except the public banner requires the bearer token below.',
    endpoint: 'Endpoint',
    statusRunning: 'running',
    statusStarting: 'starting…',
    token: 'Token',
  ...
  wizard: {
    ...
    chooseSub:
      'Catstep MD works without AI, but the agent panel and recipes need a model. Pick one path — you can change it any time in Settings → AI.',
    ...
    localNotRunningHint:
      'Install Ollama (~80 MB), launch it, then click Retry. Catstep MD does not bundle Ollama.',
  ...
  pomodoro: {
    ...
    notificationTitle: 'Catstep MD — focus session complete',
  ...
  cloudSync: {
    ...
    detectedHint:
      'Files saved here propagate automatically to your other devices via the cloud provider. Catstep MD adds cross-device session restore on top — open the same workspace on another machine and the same tabs come back.',
  ...
  githubSync: {
    heading: 'GitHub sync',
    intro:
      'Push your vault to a private GitHub repo so it follows you across devices. Your data stays on your account — Catstep MD just runs git push and git pull.',
    ...
    keychainHintBody:
      'The first time Catstep MD pushes to GitHub, macOS pops a Keychain dialog so it can read your saved token. If End-to-end encryption is on, you\'ll see a second prompt for the encryption key. Click "Always Allow" (not just "Allow") on each — this whitelists Catstep MD permanently and you\'ll never see the prompt again on this Mac. If you click "Allow" once, you\'ll be re-prompted every push.',
    proxyTitle: 'HTTPS proxy (for GFW / corporate networks)',
    proxyPlaceholder: 'http://127.0.0.1:7897 or socks5://127.0.0.1:1080',
    proxyHint:
      'Routes git push / pull through this proxy. Leave empty for direct connect. macOS GUI apps don\'t inherit shell HTTPS_PROXY env, so set it here for Catstep MD specifically. Also fixes "Operation timed out" when github.com is unreachable directly.',
    ...
    upgradeFormBody:
      'Set a passphrase. Catstep MD will derive a key (Argon2id) and re-encrypt every file before pushing again.',
  ...
  agent: {
    ...
    empty: {
      ...
      noAi: 'Configure an AI provider in Settings → Integrations → AI Provider & Models to start chatting.',
  ...
  search: {
    ...
    hitCount: '{n} hits',
    keyHint: '↑↓ navigate · ↵ open · Tab semantic search · Esc close',
    modeExact: 'Exact',
    modeSemantic: 'Semantic',
    switchToExact: 'Switch to exact search ({key})',
    switchToSemantic: 'Switch to semantic search ({key})',
    tabSwitchHint: 'Press Tab to switch mode',
  },
  views: {
    heading: 'Views',
    newView: 'New view',
